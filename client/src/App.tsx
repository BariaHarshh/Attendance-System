...
// DOUBLE CHECK THIS URL IN RENDER DASHBOARD
...
function App() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      setConnStatus('Backend Offline - Waking up...')
      setIsWakingUp(true)
      // Auto-retry every 3 seconds to force Render to wake up
  ...
  const markAttendance = async () => {
    ...
    try {
      ...
      alert('Error: Backend is still sleeping. Please wait for the green "Connected" status at the top!')
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-6 text-center">
          <div className="space-y-6 max-w-xs">
            <div className="relative">
              ...
              <Radio className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-blue-500 animate-pulse" size={32} />
            </div>
            <div className="space-y-2">
              <h3 className="text-xl font-bold">Waking up Server</h3>
              <p className="text-sm text-slate-500">Render Free Plan servers sleep after inactivity. This will take ~30 seconds. Please wait...</p>
            </div>
            <div className="text-[10px] font-mono text-blue-500/50 bg-blue-500/5 px-3 py-1 rounded-full uppercase tracking-widest">
              Status: {connStatus}
            </div>
          </div>
        </div>
            ...
            <div className="w-[375px] h-[750px] bg-slate-950 border-[8px] border-slate-800 rounded-[3rem] overflow-hidden relative shadow-[0_0_80px_rgba(0,0,0,0.5)] flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-6 pt-10">
                ...
                <div className="space-y-4 mb-8">
                  ...
                  />
                  <button onClick={markAttendance} className="w-full bg-white text-black font-bold py-4 rounded-2xl flex items-center justify-center gap-2 active:scale-95 transition-all shadow-lg">
